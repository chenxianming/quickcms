...
_e(function (E, $) {
    ...
    DropPanel.fn.initHideEvent = function () {
        ...
        E.$body.on('click', function (e) {
            ...
            if($(e.target).parents('.media_selector').length>0 || $(e.target).hasClass('media_selector')){
                return ;
            }
...
_e(function (E, $) {
    ...
    function linkContentHandler (editor, menu, $linkContent) {
        var lang = editor.config.lang;
        var $urlContainer = $(`<div style="margin:20px 10px 10px 10px;">
            <div class="media_input big" data-id="${~~(Math.random() * 100000)}">
                <div class="media_input_btn"><i class="material-icons">subscriptions</i></div>
            </div>
        </div>`);
        var $urlInput = $(`
            <input class="insert block" name="icon" type="text" placeholder="从媒体库中选择图片(请注意格式)" value="">
        `);
        $urlContainer.find('.media_input').prepend($urlInput);
        var $btnSubmit = $('<button class="right">' + lang.submit + '</button>');
        var $btnCancel = $('<button class="right gray">' + lang.cancel + '</button>');

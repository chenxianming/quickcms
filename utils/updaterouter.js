...
module.exports = function(){
    ...
    ( () => new Promise( (resolve,reject) => {
        ...
        var queryStr = `select * from ${configs.pre}category where id = 1`;
        ...
        var d = eval(`[${datas.datas}]`);
        
        categories = d[0];

        dynamicRoutes = routerStr(categories,router);
        resolve();

    } ) ).then( () => new Promise( (resolve,reject) => {
        
        var queryStr = `show tables like "${configs.pre}ctconfig_%"`;
        ...
        task.each( (item) => new Promise( (rev1,rej1) => {
            ...
            connect({
                ...
                var queryFn = con.query(queryStr,function(err,data){
                    ...
                    var model = item.replace(`${configs.pre}ctconfig_`,'');
                    modelConfigs[model] = data;
                    rev1();
                });
            });
    ...
    var modelConfigs = ${JSON.stringify(modelConfigs)};
        `;
            ...
            tasks.each( (item) => new Promise( (rev1,rej1) => {
                ...
                connect(function(con){
                    var queryFn = con.query(queryStr,function(err,data){
                        ...
                        var model = item.replace(\`${configs.pre}content_\`,'');
    ...
    function viewsQuery(req, res, next, categoriesStra) {
        ...
        dynamicRoutes.routes.forEach(function(d,i){
            //==> jsonp & api
            chunk += `
    router.get('/jsonp${d}',function(req, res, next){
        jsonpRoutes(req, res, next, '${dynamicRoutes.arr[i]}');
    });

    router.get('/jsonp${d}:query',function(req, res, next){
        jsonpQuery(req, res, next, '${dynamicRoutes.arr[i]}');
    });

    router.post('/api${d}',function(req, res, next){
        apiRoutes(req, res, next, '${dynamicRoutes.arr[i]}');
    });

    router.post('/api${d}:query',function(req, res, next){
        apiQuery(req, res, next, '${dynamicRoutes.arr[i]}');
    });
            `;
    ...
    router.get('${d}',function(req, res, next){
        viewsRoutes(req, res, next, '${dynamicRoutes.arr[i]}');
    });

    router.get('${d}:query',function(req, res, next){
        viewsQuery(req, res, next, '${dynamicRoutes.arr[i]}');
    });
            `;
...
module.exports = function(router){
    ...
    var engine = encrypter('${configs.sessionKey}',{ttl:false});
    ...
    router.get('/jsonp/getcomments',function(req,res,next){
        ...
        ( () => new Promise( (resolve,reject) => {
            var queryStr = \`select * from ${configs.pre}comments\ where model = "\${model}" and ctid = "\${id}"\`;
    ...
    router.post('/api/new-reply',function(req,res,next){
        ...
        ( () => new Promise( (resolve,reject) => {
            ...
            if(comment){
                ...
                var queryStr = \`insert into ${configs.pre}comments (ctid,title,model,datas,lastupdate) values("\${id}","\${mainTitle}","\${model}",'\${JSON.stringify(arr)}',"\${date}")\`;
    ...
    router.post('/api/extensions',function(req,res,next){
        ...
        ( () => new Promise( (resolve,reject) => {
            ...
            if(action=='query'){
                ...
                var queryStr = \`update ${configs.pre}extensions set \${input} = "\${datas}" where guid = "\${guid}"\`;
    ...
    ${chunk}
        ...
        if(fs.existsSync(`${__dirname}/dynamicRoutes.js`)){
            fs.unlinkSync(`${__dirname}/dynamicRoutes.js`);
        }
        
        fs.writeFile(`${__dirname}/dynamicRoutes.js`,chunk2,function(err,result){
            if(err){
